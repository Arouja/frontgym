...
const Orm = () => {
    ...
    const calculateOrm = (weight, reps) => {
      axios.post('http://localhost:8089/api/v1/fitness-metrics/calculate-orm', {
        ...
        .catch((error) => {
          console.error('Error calculating ORM:', error.response.data);
        });
    };
    ...
    const handleCalculateORM = () => {
      ...
      if (!isNaN(weight) && !isNaN(reps)) {
        calculateOrm(weight, reps); // Call the calculateOrm function with weight and reps
      }
    };
...
ORM stands for One Repetition Maximum. It is a measure used in weightlifting and strength training to represent the maximum amount of weight that a person can lift for a single repetition of a given exercise. 
ORM is an important metric for assessing an individual's strength and tracking progress in weight training programs.
            </p>
          </div>
        </div>
